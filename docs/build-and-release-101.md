...
## Branches
Github Actions CI builds are triggered for the following branches: `master`, `SP/**`, `HF/**`
and `GOOGLEDOCS**`. Behaviour differs by branch:
...
* `master` branch:
  - if the commit message contains the `[publish]` tag, the build will include the
...
Steps:
1. Create a new branch with the name `GOOGLEDOCS-###_release_version` from the `master` / `SP/*` /
`HF/*` branch.
...
5. Open a new Pull Request into the original `master` / `SP/*` / `HF/*` branch and wait for a green
