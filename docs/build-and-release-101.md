...
3. **Release**: Release and Deployment by publishing to Nexus and the AWS Staging bucket.
4. **Company Release**: Publishes the released `distribution` to the AWS Releases bucket.


## Branches
Github Actions CI builds are triggered for the following branches: `master`, `release/**`,
`feature/**` and `fix/**`. Behaviour differs by branch:
* `master` / `release/**` branches:
  ...
  - if the commit message contains the `[publish]` tag, the builds will include the
  _Company Release_ stage;
...
Prerequisites:
 - the `master` / `release/**` branch is green and it contains all the changes that should be
 included in the next release.

Steps:
1. Create a new branch with the name `release/version` (or a `feature/*` / `fix/*` branch) from the
...
5. Open a new Pull Request into the original `master` / `release/**` branch and wait for a green
build; the **Release** stage on the PR build will only execute a _Dry_Run_ of the release.
...
Steps:
1. On the `master` branch, push a commit whose message contains the `[publish]` tag. The
**Company Release** job uses the latest branch git tag to identify the version to be uploaded to
the S3 release bucket.
